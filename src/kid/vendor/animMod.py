    ...
    if target == "keysSel" or target == "keysIndexSel":        
        ...
        if animCurves:
            ...
            for node in animCurves:
                ...
                for n in xrange(100): # find transform node (transform) and attribute name
                    obj  = None
                    attr = None
...
def align(sourceObjs, targetObj, translate=True, rotate=True, suspend=True, onlyCurrFrame=True):
    ...
    for loopKey in keysSel:                    
        ...
        if translate:
            rotation = cmds.xform(targetObj, query=True, ws=True, rotatePivot=True)
        ...
        for loopSourceObj in sourceObjs:
            ...
            if translate:     
                ...
                cmds.xform(loopSourceObj, ws=True, rotation=(localPivot[0]*-1,localPivot[1]*-1,localPivot[2]*-1))
                ...
                cmds.move(rotation[0],rotation[1],rotation[2], loopSourceObj, relative=True, worldSpace=True)
